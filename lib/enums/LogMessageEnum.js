module.exports = {
  DEBUG_MESSAGES: {
    ...
    SET_DEVELOPMENT_MODE: '({file}): DEVELOPMET mode is set',
    ...
    NO_STORED_VARIATION:
      '({file}): No stored variation for UserId:{userId} of Campaign:{campaignTestKey} found in UserProfileService',
    ...
    GETTING_STORED_VARIATION:
      '({file}): Got stored variation for UserId:{userId} of Campaign:{campaignTestKey} as Variation: {variationId}, found in UserProfileService',
    ...
    GOT_VARIATION_FOR_USER:
      '({file}): userId:{userId} for campaign:{campaignTestKey} got variationId:{variationId} inside method:{method}',
    ...
    IMPRESSION_FOR_TRACK_USER: '({file}): impression built for track-user',
    IMPRESSION_FOR_TRACK_GOAL: '({file}): impression built for track-goal'
  },
  INFO_MESSAGES: {
    VARIATION_RANGE_ALLOCATION:
      '({file}): Campaign:{campaignTestKey} having variation:{variationId} with weight:{variationWeight} got range as: ( {start} - {end} ))',
    VARIATION_ALLOCATED: '({file}): UserId:{userId} of Campaign:{campaignTestKey} got variation: {variationId}',
    ...
    GOT_STORED_VARIATION:
      '({file}): Got stored variation:{variationId} of campaign:{campaignTestKey} for userId:{userId} from UserProfileService',
    ...
    GOT_VARIATION_FOR_USER: '({file}): userId:{userId} for campaign:{campaignTestKey} got variationId:{variationId}',
  ...
  ERROR_MESSAGES: {
    ...
    GOAL_NOT_FOUND: '({file}): Goal not found for campaign:{campaignTestKey} and userId:{userId}'
  }
};
