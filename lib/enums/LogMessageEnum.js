...
module.exports = {
  ...
  INFO_MESSAGES: {
    ...
    IMPRESSION_SUCCESS:
      '({file}): Impression event - {endPoint} was successfully received by VWO having main keys: accountId:{accountId}, userId:{userId}, campaignId:{campaignId} and vairationId:{variationId}',
  ...
  ERROR_MESSAGES: {
    ...
    ACTIVATE_API_MISSING_PARAMS:
      '({file}): "activate" API got bad parameters. It expects campaignTestKey(String) as first and userId(String) as second argument',
    ACTIVATE_API_CONFIG_CORRUPTED: '({file}): "activate" API has corrupted configuration',

    GET_VARIATION_API_MISSING_PARAMS:
      '({file}): "getVariation" API got bad parameters. It expects campaignTestKey(String) as first and userId(String) as second argument',
    GET_VARIATION_API_CONFIG_CORRUPTED: '({file}): "getVariation" API has corrupted configuration',

    TRACK_API_MISSING_PARAMS:
      '({file}): "track" API got bad parameters. It expects campaignTestKey(String) as first, userId(String) as second and goalIdentifier(String/Number) as third argument. Fourth is revenueValue(Float/Number/String) and is required for revenue goal only.',
    TRACK_API_CONFIG_CORRUPTED: '({file}): "track" API has corrupted configuration',
    TRACK_API_GOAL_NOT_FOUND:
      '({file}): Goal:{goalIdentifier} not found for campaign:{campaignTestKey} and userId:{userId}',
    TRACK_API_REVENUE_NOT_PASSED_FOR_REVENUE_GOAL:
      '({file}): Revenue value should be passed for revenue goal:{goalIdentifier} for campaign:{campaignTestKey} and userId:{userId}',
