...
class VWO {
  ...
  activate(campaignTestKey, userId) {
    ...
    let campaign = CampaignUtil.getCampaign(configFile, campaignTestKey);
    ...
    let properties = ImpressionUtil.buildEvent(configFile, campaign.id, variationId, userId);
  ...
  getVariation(campaignTestKey, userId) {
    ...
    let variationId = VariationDecider.getVariationAllotted(configFile, campaignTestKey, userId);
  ...
  track(campaignTestKey, userId, goalIdentifier, revenue) {
    ...
    if (variationId) {
      ...
      if (goal) {
        let properties = ImpressionUtil.buildEvent(configFile, campaignId, variationId, userId, goal.id, revenue);

        this.eventQueue.process(config, properties, this);
      } else {
        this.logger.log(
          LogLevelEnum.ERROR,
          LogMessageUtil.build(LogMessageEnum.ERROR_MESSAGES.GOAL_NOT_FOUND, {
            file: FileNameEnum.VWO,
            userId,
            campaignTestKey: campaignTestKey
          })
