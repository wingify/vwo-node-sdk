...
class VWO {
  ...
  constructor(config = {}) {
    ...
    if (!projectConfigManager.isValidsettingsFileAndSdkKey(config)) {
      this.logger.log(
        LogLevelEnum.ERROR,
  ...
  track(campaignTestKey, userId, goalIdentifier, revenueValue) {
    ...
    if (variationName) {
      ...
      if (goal) {
        let properties = ImpressionUtil.buildEvent(settingsFile, campaignId, variationId, userId, goal.id, revenueValue);
