...
let BatchEventsDispatcher = {
  dispatch: function(properties, callback, queryParams, authToken) {
    const parsedUrl = url.parse(Constants.HTTPS_PROTOCOL + UrlEnum.BASE_URL + UrlEnum.BATCH_EVENTS);
    HttpHandlerUtil.sendPostCall(parsedUrl, properties, callback, queryParams, authToken);
  }
};
