 ...
 */
function activate(vwoInstance, campaignKey, userId, customVariables) {
  const api = ApiEnum.ACTIVATE;

  // Check if arguments have valid data-type
  if (!ValidateUtil.areValidParamsForAPIMethod({ method: ApiEnum.ACTIVATE, campaignKey, userId, customVariables })) {
  ...
  const { variationId, variationName } = DecisionUtil.getVariation(config, campaign, campaignKey, userId);
