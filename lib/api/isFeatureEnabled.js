...
function isFeatureEnabled(vwoInstance, campaignKey, userId) {
  ...
  if (!ValidateUtil.areValidParamsForAPIMethod(ApiEnum.IS_FEATURE_ENABLED, campaignKey, userId)) {
  ...
  if (!settingsFile) {
    vwoInstance.logger.log(
      LogLevelEnum.ERROR,
      LogMessageUtil.build(LogMessageEnum.ERROR_MESSAGES.IS_FEATURE_ENABLED_API_CONFIG_CORRUPTED, {
        file
      })
    );
  ...
  const { variation, variationName, variationId } = DecisionUtil.getVariation(config, campaign, campaignKey, userId);
