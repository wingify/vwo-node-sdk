 ...
 */
function track(vwoInstance, campaignKey, userId, goalIdentifier, revenueValue) {
  const api = ApiEnum.TRACK;
  // Check if arguments have valid data-type
  if (!ValidateUtil.areValidParamsForAPIMethod(ApiEnum.TRACK, campaignKey, userId, goalIdentifier)) {
  ...
  let { variationId, variationName } = DecisionUtil.getVariation(config, campaign, campaignKey, userId);
