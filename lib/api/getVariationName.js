...
const file = FileNameEnum.GetVariationName;
 ...
 */
function getVariationName(vwoInstance, campaignKey, userId) {
  const api = ApiEnum.GET_VARIATION_NAME;

  // Check if arguments have valid data-type
  if (!ValidateUtil.areValidParamsForAPIMethod(ApiEnum.GET_VARIATION_NAME, campaignKey, userId)) {
  ...
  const { variationName } = DecisionUtil.getVariation(config, campaign, campaignKey, userId);

  return variationName;
}

module.exports = getVariationName;
