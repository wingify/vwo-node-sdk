...
let CampaignUtil = {
  ...
  isCampaignRunning: (projectConfig, campaignId) => {
    return CampaignUtil.getCampaignStatus(projectConfig, campaignId) === 'running';
  },
  /**
  ...
  getCampaignVariation: (config, campaignId, variationId) => {
    let configObj = config.configFile;

    if (!configObj || !campaignId || !variationId) {
      // log error
      return;
    }

    let campaign = CampaignUtil.getCampaign(configObj, campaignId);
