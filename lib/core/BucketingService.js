...
const SegmentEvaluator = require('./SegmentEvaluator');
...
let BucketingService = {
  ...
  isUserPartOfCampaign: (userId, campaign) => {
    ...
    if (SegmentEvaluator.evaluate(conditions)) {
    ...
    logger.log(
      LogLevelEnum.INFO,
      LogMessageUtil.build(LogMessageEnum.INFO_MESSAGES.SegmentEvaluator_CONDITION_NOT_MET, {
        file: FileNameEnum.BucketingService,
        userId
      })
    );
