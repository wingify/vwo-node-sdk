...
module.exports = {
  ...
  INFO_MESSAGES: {
    PRESEG_CONDITION_NOT_MET:
      '({file}): User ID:{userId} does not become part of campaign because of not meeting pre-seg conditions',
    ...
    EMPTY_SEGMENTS_OBJECT:
      '({file}): In {api} API, customVariables are passed for campaignKey:{campaignKey} and userId:{userId} but campaign has no pre-segmentation',
    MISSING_customVariables:
      '({file}): In {api} API, customVariables are not passed for campaignKey:{campaignKey} and userId:{userId} and campaign has pre-segmentation',
    USER_PASSED_PRE_SEGMENTATION:
      '({file}): In {api} API, userId:{userId} of campaignKey:{campaignKey} with customVariables:{customVariables} passed pre segmentation',
    USER_FAILED_PRE_SEGMENTATION:
      '({file}): In {api} API, userId:{userId} of campaignKey:{campaignKey} with customVariables:{customVariables} failed pre segmentation',
    PRE_SEGMENTATION_ERROR:
      '({file}): In {api} API, Error while segmenting the userId:{userId} of campaignKey:{campaignKey} with customVariables:{customVariables}. Error message: {e}'
  },
  WARNING_MESSAGES: {},
  ERROR_MESSAGES: {
    ACTIVATE_API_CONFIG_CORRUPTED: '({file}): "activate" API has corrupted configuration',
    ACTIVATE_API_MISSING_PARAMS:
      '({file}): "activate" API got bad parameters. It expects campaignKey(String) as first and userId(String) as second argument',
    ...
    GET_FEATURE_VARIABLE_MISSING_PARAMS: `({file}): "getFeatureVariableValue" API got bad parameters. It expects campaignKey(String) as first, variableKey(String) as second and userId(String) as third argument`,
    GET_VARIATION_API_CONFIG_CORRUPTED: '({file}): "getVariation" API has corrupted configuration',
    GET_VARIATION_API_MISSING_PARAMS:
      '({file}): "getVariation" API got bad parameters. It expects campaignKey(String) as first and userId(String) as second argument',
    IMPRESSION_FAILED: '({file}): Impression event could not be sent to VWO - {endPoint}',
    ...
    IS_FEATURE_ENABLED_API_MISSING_PARAMS:
      '({file}): "isFeatureEnabled" API got bad parameters. It expects Campaign(String) as first and userId(String) as second argument',
    ...
    TRACK_API_MISSING_PARAMS:
      '({file}): "track" API got bad parameters. It expects campaignKey(String) as first, userId(String) as second and goalIdentifier(String/Number) as third argument. Fourth is revenueValue(Float/Number/String) and is required for revenue goal only.',
