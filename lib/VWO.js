...
class VWO {
   ...
   */
  activate(campaignKey, userId, customVariables) {
    try {
      let self = this;

      return api.activate(self, campaignKey, userId, customVariables);
    } catch (err) {
   ...
   * @return {String|null}             If variation is assigned then variation-name otherwise null in case of user not becoming part
   */
  getVariationName(campaignKey, userId, customVariables) {
    try {
      let self = this;

      return api.getVariation(self, campaignKey, userId, customVariables);
    } catch (err) {
      this.logger.log(LogLevelEnum.ERROR, err.message);
      return null;
    }
   ...
   */
  track(campaignKey, userId, goalIdentifier, revenueValue, customVariables) {
    try {
      let self = this;

      return api.track(self, campaignKey, userId, goalIdentifier, revenueValue, customVariables);
    } catch (err) {
      this.logger.log(LogLevelEnum.ERROR, err.message);
      return false;
    }
   ...
   */
  isFeatureEnabled(campaignKey, userId, customVariables) {
    try {
      let self = this;

      return api.isFeatureEnabled(self, campaignKey, userId, customVariables);
    } catch (err) {
      this.logger.log(LogLevelEnum.ERROR, err.message);
      return false;
    }
   ...
   */
  getFeatureVariableValue(campaignKey, variableKey, userId, customVariables) {
    try {
      let self = this;

      return api.getFeatureVariableValue(self, campaignKey, variableKey, userId, customVariables);
    } catch (err) {
      this.logger.log(LogLevelEnum.ERROR, err.message);
      return null;
    }
